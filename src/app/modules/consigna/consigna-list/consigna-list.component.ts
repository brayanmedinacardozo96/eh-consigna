...
export class ConsignaListComponent implements OnInit {
  ...
  verMapa(row) {
    var objJson = JSON.parse(row);
    var dataMapa = {
      listaElemento: objJson.url.indicador.listaElemento,
      indicador: objJson.url.indicador.indicador
    };
    ...
    window.open(environment.urlEhmapV2 + '?' + data + '&key=' + null, "MsgWindow", 'width=' + width + ',height=' + height + ',top=' + y + ',left=' + x + ',toolbar=no,resizable=no');
